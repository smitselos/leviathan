  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', height:'100vh', width:'100vw', overflow:'hidden' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:8, padding:'8px 12px', borderBottom:'1px solid #ebebeb', background:'#fff', flexShrink:0 }}>
        ...
        <button onClick={function() { window.open(fileUrl, '_blank'); }} style={ibtn} title="Νέα καρτέλα">{'↗'}</button>
      </div>
      ...
      <iframe src={fileUrl} style={{ flex:1, border:'none', width:'100%' }} title={name} />
    </div>
  );
}
