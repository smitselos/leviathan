...
export default function StudentPage() {
  ...
  const [visibilityInfo, setVisibilityInfo] = useState(null); // αρχείο για «πού στάλθηκε»
                          ...
                          : <div style={{ display:'flex', gap:6, flexShrink:0 }}>
                              {f.visibility && f.visibility !== 'none' && myEmail && (teacher === myEmail || !teacher) && (
                                <button onClick={e=>{e.stopPropagation();setVisibilityInfo(f);}}
                                  style={{ background:'transparent', border:'1.5px solid #e0e0e0', borderRadius:10, padding:'6px 10px', fontSize:13, cursor:'pointer', color:'#6b6b80', title:'Πού στάλθηκε' }}>
