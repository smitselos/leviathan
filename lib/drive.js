...
// ── Native Google αρχεία (Docs/Slides/Sheets/Drawings) ─────────
// Δεν έχουν κατάληξη ούτε δυαδικό περιεχόμενο — εξάγονται ΑΠΕΥΘΕΙΑΣ
// σε PDF με files.export, χωρίς ενδιάμεσο αντίγραφο-μετατροπή.
const LV_GOOGLE_NATIVE = new Set([
...
export function isGoogleNative(mime) {
  return LV_GOOGLE_NATIVE.has(mime || '');
}
...
export async function ensurePdfCopy(drive, sourceId, sourceName, sourceMime) {
  const ext = (sourceName.match(/\.([^.]+)$/) || [])[1]?.toLowerCase();
  const gMime = LV_OFFICE_EXT[ext];
  const native = !gMime && isGoogleNative(sourceMime);
  if (!gMime && !native) return null; // ούτε Office ούτε native Google — δεν χρειάζεται μετατροπή

  // Native: το όνομα δεν έχει κατάληξη — μην κόψεις τυχόν «.1» κ.λπ.
  const baseName = native ? sourceName : sourceName.replace(/\.[^.]+$/, '');
  ...
  // Office: copy ως Google native (convert) → export PDF. Native Google: export ΑΠΕΥΘΕΙΑΣ.
  let tempId = null;
  try {
    let exportId = sourceId;
    if (!native) {
    ...
    const exp = await drive.files.export(
      { fileId: exportId, mimeType: 'application/pdf' },
...
export async function loadRegistry(drive) {
  ...
  try {
    ...
    return {
      ...
      // ΚΡΙΣΙΜΟ: τα Δίκτυα Κειμένων αποθηκεύονται εδώ (βλ. api/networks.js) —
      // χωρίς αυτή τη γραμμή, κάθε ανάγνωση τα «ξεχνούσε» και κάθε αποθήκευση τα έσβηνε.
...
export async function saveRegistry(drive, registry) {
  const payload = {
    ...
    networks: registry.networks || [], // Δίκτυα Κειμένων — να ΓΡΑΦΟΝΤΑΙ, όχι να πετιούνται
  };
